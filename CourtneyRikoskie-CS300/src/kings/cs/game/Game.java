...
/**
 * This class is the main class of the "Cat" application. "Cat" is a text-based
 * adventure game.
 * 
...
public class Game {
	...
	private void goRoom(Command command) {
		if (!command.hasSecondWord()) {
			...
			if (doorway == null) {
			...
			} else if (currentRoom.getName().equals("Front Porch") && direction.equals("down")
					&& clearing.isInRoom("hidden barrier rune")) {
				writer.print(
						"You step into the forest, determined and ready to face the world, but your confidence isn't well-placed.");
				writer.println(
						" Master comes out of the trees, a severe frown on her face. You probably should have gotten rid of that barrier somehow.");
				writer.println();

				wantToContinue = false;
			} else if (currentRoom.getName().equals("In Front of Bridge") && direction.equals("east")
					&& !player.isInInventory("citizenship card")) {
				writer.println("Guardsman: Turn around, Faye. And don't come back until you're a proper citizen.");
			} else if (currentRoom.getName().equals("South Path") && direction.equals("south")
					&& !player.isInInventory("illuminated bulb") && !currentRoom.isInRoom("illuminated bulb")) {
				writer.println(
			...
			} else if (currentRoom.getName().equals("Fairsway Weapons")
					&& !(weapons.isInRoom("broadsword") || weapons.isInRoom("duplicate broadsword"))) {
				writer.println("Tave: I don't mind a little mischief, but that sword doesn't leave this store.");
			} else if (currentRoom.getName().equals("North Path") && direction.equals("up")
					&& player.isInInventory("cat")) {
				printWin();
				wantToContinue = false;
			} else {
				...
				if (newRoom.getName().equals("South Path")
						&& (player.isInInventory("illuminated bulb") || newRoom.isInRoom("illuminated bulb"))) {
					newRoom.setActive(1);
				}
				...
				if (newRoom.getName().equals("Welcome to the Citizen District") && !weapons.isInRoom("broadsword")) {
					...
					taveSecondCon.addReply("hi",
							"Someone stole my broadsword.\n\n\tA: I'm sorry\n\tB: Who would do that?\n");
	...
	private void take(Command command) {
		if (!command.hasSecondWord()) {
			...
			if (currentRoom.isInRoom(itemName)) {
				...
				if (added) {
					takeSuccess(item, currentRoom);
				} else {
					takeItemWeightFailure(item);
				}
			} else {
	...
	/**
	 * Executes the actions and checks needed if the item was taken.
	 * 
	 * @param item The item that was taken.
	 * @param currentRoom The current room.
	 */
	private void takeSuccess(Item item, Room currentRoom) {
		String result = "Taken.";
		...
		if (item instanceof Ingredient) {
			...
			result = checkIngredientState(ingredient, currentRoom);
		} else {
	...
	/**
	 * Checks the state of the ingredient (the room it is in, the number in its group) and takes the appropriate actions.
	 * 
	 * @param ingredient The ingredient that was taken.
	 * @param currentRoom The current room.
	 * @return The result.
	 */
	private String checkIngredientState(Ingredient ingredient, Room currentRoom) {
		String result = "Taken.";
		
		if (isTakingLastHerbs(ingredient, currentRoom)) {
			currentRoom.setActive(1);
		}
		if (hasTakenAllOfHerb(ingredient)) {
			currentRoom.removeItem(ingredient.getName());
		}
		if (hasLostByTakingTooManyHerbs(currentRoom, ingredient.getName())) {
			result = executeTakeLoss();
		}
	...
	/**
	 * Executes the needed actions and checks if the item could not be taken.
	 * 
	 * @param item The item that could not be taken.
	 */
	private void takeItemWeightFailure(Item item) {
	...
	/**
	 * Checks if player is in the backyard and taking one of the limited ingredients
	 * existing there.
	 * 
	 * @param ingredient
	 *            The ingredient that the player is taking
	 * @param currentRoom
	 *            The room that the player is in
	 * @return Whether or not the player is in the backyard, taking one of the
	 *         limited ingredients
	 */
	private boolean isTakingLastHerbs(Ingredient ingredient, Room currentRoom) {
		return ingredient.getNumberInGroup() < 3 && currentRoom.getName().equals("Backyard")
				&& !ingredient.getName().equals("orreamin");
	}

	/**
	 * Checks if the number of an ingredient falls to 0.
	 * 
	 * @param ingredient
	 *            The ingredient to remove.
	 */
	private boolean hasTakenAllOfHerb(Ingredient ingredient) {
		return ingredient.getNumberInGroup() == 0;
	}

	/**
	 * Checks if the player has lost by taking the last of the limited number of herbs in the backyard.
	 * 
	 * @param currentRoom The current room.
	 * @param itemName The name of the item that the player has taken.
	 * @return If the player has lost
	 */
	private boolean hasLostByTakingTooManyHerbs(Room currentRoom, String itemName) {
		return currentRoom.getName().equals("Backyard") && !currentRoom.isInRoom(itemName);
	}
	
	/**
	 * Executes the necessary actions if the player has lost by taking the last of the limited number of herbs in the backyard.
	 * 
	 * @param result The result to print.
	 */
	private String executeTakeLoss() {
		writer.print("You've made quite a mess of the garden, and you're master is sure to notice.");
		writer.println(
				" She'll definitely think that you're irresponsible. Even if you do get your cat back, what's the point?");
		wantToContinue = false;
		return "";
	}
	...
	private void pack(Command command) {
		if (!command.hasSecondWord()) {
			...
			if (!doesItemExist(currentRoom, itemName)) {
				writer.println("You can't pack an item you don't have.");
			} else {
				Item item = findItem(currentRoom, itemName);
				...
				if (item.getWeight() > Player.MAX_WEIGHT) {
					...
					if (!doesItemExist(currentRoom, response)) {
						writer.println("You don't see that container anywhere.");
					} else {
						packInGivenContainer(currentRoom, response, item);
					}
				}
			}
		}
	...
	/**
	 * Checks if the ingredient is from the backyard.
	 * 
	 * @param currentRoom The current room.
	 * @param ingredientName The name of the ingredient.
	 * @return If the ingredient is from the backyard.
	 */
	private boolean isIngredientFromBackyard(Room currentRoom, String ingredientName) {
		return currentRoom.getName().equals("Backyard")&& !player.isInInventory(ingredientName);
	}
	
	/**
	 * Packs the item in the given container.
	 * 
	 * @param currentRoom The current room.
	 * @param response The container.
	 * @param item The item.
	 */
	private void packInGivenContainer(Room currentRoom, String response, Item item) {
		Item container = findItem(currentRoom, response);
		...
		if (!(container instanceof Container) || container instanceof PotionContainer) {
			...
			packWithHerbContainer(item, currentRoom, herbContainer);
		} else {
			Container aContainer = (Container) container;

			packWithNormalContainer(item, currentRoom, aContainer);
		}
	}
	
	/**
	 * Packs the item in a regular container, if possible.
	 * 
	 * @param item The item.
	 * @param currentRoom The current room.
	 * @param aContainer The container.
	 */
	private void packWithNormalContainer(Item item, Room currentRoom, Container aContainer) {
		if (item instanceof Ingredient) {
			writer.println("You probably shouldn't try to put that in there.");
		} else {
			packNonIngredient(currentRoom, item, aContainer);
		}
	}
	
	/**
	 * Packs the item in an herb container, if possible.
	 * 
	 * @param item The item.
	 * @param currentRoom The current room.
	 * @param herbContainer The herb container.
	 */
	private void packWithHerbContainer(Item item, Room currentRoom, HerbContainer herbContainer) {
		if (!(item instanceof Ingredient)) {
			writer.println("You probably shouldn't try to put that in there.");
		} else {
			packIngredient(item, currentRoom, herbContainer);
		}
	}
	
	/**
	 * Packs an item that is not an ingredient.
	 * 
	 * @param currentRoom The current room.
	 * @param item The item.
	 * @param aContainer The container.
	 */
	private void packNonIngredient(Room currentRoom, Item item, Container aContainer) {
		if (!(currentRoom.isInRoom(item.getName()) && player.isInInventory(aContainer.getName()))) {
			packItemFromInventory(aContainer, item);
		} else {
			packRoomItemIntoInventoryContainer(item, currentRoom, aContainer);
		}
	}
	
	/**
	 * Pack an item that is an ingredient.
	 * 
	 * @param item The item.
	 * @param currentRoom The current room.
	 * @param herbContainer The herb container.
	 */
	private void packIngredient(Item item, Room currentRoom, HerbContainer herbContainer) {
		Ingredient ingredient = (Ingredient) item;
		String ingredientName = ingredient.getName();

		if (currentRoom.isInRoom(ingredientName) && player.isInInventory(herbContainer.getName())) {
			if (isIngredientFromBackyard(currentRoom, ingredientName)) {
				writer.println("You probably shouldn't rip out all the herbs at once.");
			} else {
				packRoomItemIntoInventoryContainer(ingredient, currentRoom, herbContainer);
			}
		} else {
			if (isIngredientFromBackyard(currentRoom, ingredientName)) {
				writer.println("You probably shouldn't rip out all the herbs at once.");
			} else {
				packItemFromInventory(herbContainer, ingredient);
			}
		}
	}
	
	/**
	 * Removes item from room and adds it to specified container.
	 * 
	 * @param currentRoom
	 *            The current room
	 * @param aContainer
	 *            The container in which to pack the item
	 * @param item
	 *            The item to be packed
	 */
	private void packItemFromRoom(Room currentRoom, Container aContainer, Item item) {
		currentRoom.removeItem(item.getName());
	...
	/**
	 * Removes item from inventory and adds it to specified container.
	 * 
	 * @param currentRoom
	 *            The current room
	 * @param aContainer
	 ...
	 *            The item to be packed
	 */
	private void packItemFromInventory(Container aContainer, Item item) {
		writer.println("Packed.");
		aContainer.addItem(item);
		score += item.getPointValue();
	...
	/**
	 * Packs an item from the current room into a container in the player inventory.
	 * 
	 * @param item
	 *            The item to pack.
	 * @param currentRoom
	 ...
	 *            The container
	 */
	private void packRoomItemIntoInventoryContainer(Item item, Room currentRoom, Container aContainer) {
		if (item.getWeight() + player.getTotalWeight() > Player.MAX_WEIGHT) {
			writer.println("You try to pick it up, but you're already carrying so much.");
		} else {
			packItemFromRoom(currentRoom, aContainer, item);
		}
	}

	/**
	 * Gets the item from either the player inventory or current room, if possible.
	 * 
	 * @param currentRoom
	 *            The current room.
	 * @param itemName
	 ...
	 */
	private Item findItem(Room currentRoom, String itemName) {
		Item item = null;
	...
	/**
	 * Checks if the specified item exists in the player inventory or current room.
	 * 
	 * @return If the item exists in player inventory or current room.
	 */
	public boolean doesItemExist(Room currentRoom, String itemName) {
		boolean found = false;
